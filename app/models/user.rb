class User < ApplicationRecord
  ...
  validates :username, allow_blank: true, uniqueness: true, length: {minimum: 4, maximum: 150}
  validates :email, presence: true, uniqueness: true, length: {minimum: 7, maximum: 180}
  validates :email, format: { with: URI::MailTo::EMAIL_REGEXP }
  validates :password, length: { minimum: 6 }, if: -> { new_record? || !password.nil? }
  validates :mobile, uniqueness: true, presence: true, numericality: true, length: {minimum: 10, maximum: 15}
