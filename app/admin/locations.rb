ActiveAdmin.register Location do
  ...
  form do |f|
    f.inputs do
      f.input :vendor_id, as: :select, :prompt => "Select Vendor", collection: Vendor.all.collect {|ven| [ven.name, ven.id] }
      f.input :delivery_executive_id, as: :select, :prompt => "Select Delivery Executive", collection: DeliveryExecutive.all.collect {|d_exe| [d_exe.name, d_exe.id] }
      f.input :name
      f.input :start_point
      f.input :end_point
      f.input :latitude
