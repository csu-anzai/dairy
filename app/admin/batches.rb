ActiveAdmin.register Batch do
  ...
  form do |f|
    f.inputs do
      f.input :user_id, as: :select, :prompt => "☻Ⓢ Select Supplier", collection: Supplier.all.collect {|sup| [" ☻ #{sup.name} , ☎ #{sup.mobile}", sup.id] }
      f.input :item_variant_id, as: :select, :prompt => "Ⓥ Select Variant", collection: ItemVariant.all.collect {|i_var| [i_var.title, i_var.id] }
      f.input :code
